...
#						h2o=value.pts["H2O(WT%)"]
					...
					if l[0][0]==0 and l[1][0]==0:
						...
						if symbol=="d":
							...
							plt.plot(l[0][1],F-value.katz_mf_kelley_solidus(comp,0.12,l[0],h2o),color=v_col,markersize=30,marker=symbol,label="Kelley" if i==0 else "")
						elif symbol=="s":
							ti_corrected=value.get_ti_corrected(0.91,h2o,0.25)
							l=value.lee_pt_decimal(comp,h2o,[0.25,0.25])
							plt.plot(l[0][1],value.quick_mf(ti_corrected,0.09,0.123)-value.katz_mf_kelley_solidus(comp,0.12,l[0],h2o),color='c',markersize=30,marker=symbol,label="My samples" if i==0 else "")
					i+=1			
...
#		plt.scatter(0,0,label=name,marker="s",edgecolor="g",facecolors=col)
	...
	fig1.savefig("%skatz_mf_diff_kelley_sol_vs_P.png"%(directory),bbox_inches='tight',dpi=(149))
