				...
				for key,value in samps.iteritems():
					...
					if l[0][0]==0 and l[1][0]==0:
						...
						if symbol=="d":
							...
							plt.plot(l[0][1],F-value.kelley_mf_arc_with_cpx_out(0.91,l[0],4,0.12),color=v_col,markersize=30,marker=symbol,label="Kelley" if i==0 else "")
						elif symbol=="s":
							ti_corrected=value.get_ti_corrected(0.9,h2o,0.25)
							l=value.lee_pt_decimal(comp,h2o,[0.25,0.25])
							plt.plot(l[0][1],value.quick_mf(ti_corrected,0.09,0.123)-value.kelley_mf_arc_with_cpx_out(0.91,l[0],4,0.12),color='c',markersize=30,marker=symbol,label="My samples" if i==0 else "")
					i+=1			
	plt.title("MF difference vs pressure, kelley frac with cpx out, assumed cpx 0.12")
...
#		plt.scatter(0,0,label=name,marker="s",edgecolor="g",facecolors=col)
	...
	fig1.savefig("%smf_diff_vs_P_with_cpx_out.png"%(directory),bbox_inches='tight',dpi=(149))
