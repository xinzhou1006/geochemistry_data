#Plot mf difference vs P, to see if deeper samples match better. No cpx out in Katz mf calc
import numpy as np
...
#						h2o=value.pts["H2O(WT%)"]
					...
					if l[0][0]==0 and l[1][0]==0:
						...
						if symbol=="d":
							...
							katz_mf=value.katz_mf_without_cpx_out(comp,0.12,l[0],h2o)
					...
					i+=1			
	plt.title("Kelley, katz MF, no cpx out")
...
#		plt.scatter(0,0,label=name,marker="s",edgecolor="g",facecolors=col)
	...
	fig1.savefig("%skatz_kelley_mf_vs_P_no_katz_cpx_out.png"%(directory),bbox_inches='tight',dpi=(149))
