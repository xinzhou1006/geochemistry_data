			...
			for h2o in [4]:
				...
				for comp in [0.9]:
					...
					if symbol=="d":
						...
						plt.plot(l[0][0],l[0][1],markersize=20,marker=symbol,c="r",label="Kelley samples" if i==0 else "")
						i+=1
					elif symbol=="s":
						l=value.lee_pt_decimal(comp,h2o,[0.25,0.25])
						plt.plot(l[0][0],l[0][1],markersize=20,marker=symbol,c=h2o_colours[h2o],label="My samples" if i==0 else "")
						i+=1
...
#					num_in_range[h2o]+=1
	...
	if sub_zone=="South_Vanuatu" or sub_zone=="Central_Aleutians" or sub_zone=="West_Aleutians":
		H+=0.5
	
	plt.annotate("8 wt% H$_2$O solidus" ,xy = (1200,2.5),fontsize=20,color="blue",rotation=315)
	plt.annotate("Dry solidus" ,xy = (1590,5),fontsize=20,rotation=310)
	plt.annotate("Slab surface depth beneath arc" ,xy = (H_annotate_locs[sub_zone],H), xytext = (-20, 30),textcoords = 'offset points',fontsize=20)
	plt.annotate("Crustal Thickness" ,xy = (1560,cr_thickness), xytext = (-20, 20),textcoords = 'offset points',fontsize=20)
	#adiabats
	...
	if sub_zone=="Tonga":
		plt.annotate("Hollow - Mg# host = 0.92" ,xy = (1470,1.5), xytext = (1488, 1.67),fontsize=30,arrowprops=dict(facecolor='black', shrink=0.05))
		plt.annotate("Filled - Mg# host = 0.90" ,xy = (1400,1), xytext = (1488, 1),fontsize=30,arrowprops=dict(facecolor='black', shrink=0.05))
